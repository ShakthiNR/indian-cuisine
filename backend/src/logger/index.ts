...
const logger = createLogger({
    level: 'info',
    format: combine(label({ label: "adapt-ready-api"}), timestamp({format: 'YYYY-MM-DD HH:mm:ss'}), myFormat),
    transports: [
        ...
        new transports.File({
            ...
            level: "error",
            format: format.combine( label({ label: "adapt-ready-api"}), timestamp({format: 'YYYY-MM-DD HH:mm:ss'}), myFormat)
